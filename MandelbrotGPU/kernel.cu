...
__global__ void kernel(pfc::BGR_4_t * const p_dst, 
	...
	// TODO: set real and imag max/min
	//double zoom_f = 0.95;
	//int val = t / (bmp_width*bmp_height);

	real_min = point_real - (point_real - real_min) * pow(zoom_factor, image_number);
	real_max = point_real + (real_max - point_real) * pow(zoom_factor, image_number);
	imag_max = point_imag + (imag_max - point_imag) * pow(zoom_factor, image_number);
	imag_min = point_imag - (point_imag - imag_min) * pow(zoom_factor, image_number);
