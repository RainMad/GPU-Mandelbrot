//           $Id: pfc_bitmap_3.h 37996 2018-10-28 15:16:00Z p20068 $
//          $URL: https://svn01.fh-hagenberg.at/bin/cepheiden/vocational/teaching/ESD/SPS3/2015-WS/Ablauf/src/Filters/src/helpers/pfc_bitmap_3.h $
//     $Revision: 37996 $
...
#define PFC_BITMAP_VERSION "3.1.0"
   ...
   public:
     ...
     ~bitmap () = default;
      ...
      void from_file (std::string const & name) {
         ...
         ok = ok && in.good ();
         ok = ok && read (in, file_header).good ();
         ok = ok && read (in, info_header).good ();
      ...
      void to_file (std::string const & name) const {
         ...
         if (!out.good () ||
             !write (out, m_file_header).good () ||
             !write (out, m_info_header).good () ||
...
//       std::vector <pixel_file_t> buffer      (chunk_size);
         ...
         for (auto & pel : pixel_span) {
            ...
            if ((i == 0) && read (in, buffer_span).good ()) {
...
//       std::vector <pixel_file_t> buffer      (chunk_size);
         ...
         for (auto const & pel : pixel_span) {
            ...
            if ((i == chunk_size) && write (out, buffer_span).good ()) {
               processed += chunk_size;
            }
